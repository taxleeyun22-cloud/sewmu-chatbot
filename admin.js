...
async function openMyTodos(){
  ...
  /* 달력 초기값 = 이번 달 + 오늘 선택. 추가폼 날짜도 오늘로 자동 세팅 */
  const t=_todayKST();
...
/* 전체화면 ⟷ 모달 스위칭 */
function toggleMtFull(){ _mtFull=!_mtFull; _applyMtFull(); _renderMyTodos(); }
function _applyMtFull(){
  const card=document.querySelector('#myTodosModal .modal-card'); if(!card)return;
  if(_mtFull){
...
function mtSelectDay(d){
  _mtSelDate=d;
  if($g('mtNewDue'))$g('mtNewDue').value=d;
  _renderMyTodos();
  if($g('mtNewContent'))$g('mtNewContent').focus();
}
...
async function addPersonalTask(){
  ...
  try{
    ...
    if(due && /^\d{4}-\d{2}-\d{2}$/.test(due)){
      _mtCalY=parseInt(due.substring(0,4),10);
      _mtCalM0=parseInt(due.substring(5,7),10)-1;
      _mtSelDate=due;
    }
    await loadMyTodos();
  }catch(err){alert('오류: '+err.message)}
...
function _fpFieldsHtml(p){
  ...
  return '<div style="display:flex;gap:10px;flex-wrap:wrap;margin-top:4px;color:var(--text-sub)">'
    +keys.map(function(k){
      return '<span>'+e(_FJ_LABEL[k]||k)+' <b>'+e(_fjNum(f[k]))+'</b></span>';
    }).join('')+'</div>';
}
function _fpRender(){
