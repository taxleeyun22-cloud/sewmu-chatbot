...
async function openMyTodos(){
  ...
  /* 달력 초기값 = 이번 달 + 오늘 선택 */
  const t=_todayKST();
  _mtCalY=parseInt(t.substring(0,4),10);
  _mtCalM0=parseInt(t.substring(5,7),10)-1;
  _mtSelDate=t;
  _syncMtViewUI();
  await loadMyTodos();
}
...
function mtSelectDay(d){ _mtSelDate=d; _renderMyTodos(); }
/* 할일 출처 판별: 업체 > 거래처 > 방 > 개인 (room_id '__none__' 은 방 아님) */
function _mtSource(m){
  const realRoom=(m.room_id && m.room_id!=='__none__')?m.room_id:'';
  if(m.business_name) return {k:'biz', name:m.business_name};
  const cust=m.customer_name||m.customer_nickname||'';
  if(cust) return {k:'cust', name:cust};
  if(realRoom) return {k:'room', name:m.room_name||realRoom, roomId:realRoom};
  return {k:'personal', name:'개인'};
}
...
function _renderMtCalendar(list){
  const Y=_mtCalY, M0=_mtCalM0;
  if($g('mtCalTitle'))$g('mtCalTitle').textContent=Y+'년 '+(M0+1)+'월';
  ...
  const cells=[];
  for(let i=0;i<startDow;i++){ cells.push({dt:new Date(Y,M0,1-(startDow-i)), dim:true}); }
  ...
  cells.forEach((c)=>{
    ...
    let cell='<div onclick="mtSelectDay(\''+ds+'\')" style="min-height:90px;border-right:1px solid var(--gray-100);border-bottom:1px solid var(--gray-100);padding:5px 5px 7px;cursor:pointer;'+(c.dim?'background:#fafafa;':'')+(isSel?'box-shadow:inset 0 0 0 2px var(--of-primary);':'')+'">';
    ...
    if(evs.length>3) cell+='<div style="font-size:.66em;color:var(--gray-500);padding:0 4px">+'+(evs.length-3)+'개</div>';
    cell+='</div>';
    html+=cell;
  });
  html+='</div></div>';
...
async function openRoomMemos(){
  ...
  /* Phase R2-8 (M18-c 2026-05-05): 메모 대상 select 채우기 */
...
async function submitMemo(){
  ...
  try{
    ...
    if(_memoEditingId){
      ...
      /* Phase R2-8 (M18-c 2026-05-05): 대상 select 분기 — 담당자 / 업체 / 거래처 */
...
/* 거래처 재무 데이터 — 제거 (2026-05-17 사장님: "재무 싹날리자 — 신고검토표 작성하며 그걸 토대로").
 * openFinancePanel/loadFinanceRows/saveFinanceRow/deleteFinanceRow 등 일체 폐기.
 * 재무는 신고검토표(admin-filing-review.js) 워크플로로 일원화. /api/admin-finance 는 비파괴 위해 유지(미사용). */
...
window.addEventListener('popstate', function(e){
  try{
    ...
    var roomFromState = state.room || null;
    var custFromHash = null;
    var roomFromHash = null;
    ...
    if(!t){
      ...
      if(mRoom) roomFromHash = decodeURIComponent(mRoom[1]);
    }
    var targetCust = custFromState || custFromHash;
    var targetRoom = roomFromState || roomFromHash;
    ...
    /* Phase #7 적용 (2026-05-06): room=ID 있으면 상담방 자동 진입 */
    if(targetRoom && typeof openRoom === 'function'){
      if(typeof currentRoomId === 'undefined' || currentRoomId !== targetRoom){
        setTimeout(function(){ openRoom(targetRoom, {fromPopstate:true}); }, 100);
      }
    }
 ...
 * Phase #7 적용 (2026-05-06): deep link 강화
 *  - #tab=users&cust=N → 자동 거래처 dashboard
 *  - #tab=rooms&room=ID → 자동 상담방 진입 */
(function(){
  try{
    var mTab = location.hash.match(/^#tab=(\w+)/);
    var mCust = location.hash.match(/[#&]cust=(\d+)/);
    ...
    if(mTab && typeof tab === 'function'){
      ...
      var iv = setInterval(function(){
        ...
        if(mainView && mainView.style.display !== 'none'){
          ...
          if(mRoom && typeof openRoom === 'function'){
            setTimeout(function(){ openRoom(decodeURIComponent(mRoom[1]), {fromPopstate:true}); }, 400);
          }
        } else if(attempt > 50){
          clearInterval(iv);
        }
