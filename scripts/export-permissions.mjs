...
/* 옛 admin.html public/ + apps/admin (Next.js wrapper) 양쪽 으로 export */
const outputs = [
  join(repoRoot, 'public/permissions.json'),
  join(repoRoot, 'functions/api/_permissions.json'),
  join(repoRoot, 'apps/admin/public/permissions.json'),
  join(repoRoot, 'apps/admin/functions/api/_permissions.json'),
];
 ...
 * apps/admin 도 같은 mirror — Next.js wrapper 가 옛 functions/api/ 통째 import. */
const jsModuleOutputs = [
  join(repoRoot, 'functions/api/_permissions-data.js'),
  join(repoRoot, 'apps/admin/functions/api/_permissions-data.js'),
];
...
`;
for (const jsModulePath of jsModuleOutputs) {
  mkdirSync(dirname(jsModulePath), { recursive: true });
